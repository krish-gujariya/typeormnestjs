...
export class NewMigration1719216130070 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`testcases\` (\`id\` int NOT NULL AUTO_INCREMENT, \`problem_id\` int NOT NULL, \`input\` varchar(255) NOT NULL, \`output\` varchar(255) NOT NULL, \`visibility\` tinyint NOT NULL, \`problemId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `ALTER TABLE \`problems\` ADD \`time_limit\` int NOT NULL DEFAULT '0'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`problems\` ADD \`memory_limit\` int NOT NULL DEFAULT '0'`,
    );
    await queryRunner.query(
