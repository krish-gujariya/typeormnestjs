...
export class SubmissionsController {
  ...
  @ApiCreatedResponse({
    status: 201,
    description: "Problem's sollution submitted successfully",
  })
  ...
  @ApiCreatedResponse({
    description: 'All submission of user of particular problem retrived...',
  })
  @ApiBadRequestResponse({ description: 'No record found...' })
  @ApiInternalServerErrorResponse({ description: 'Something went wrong...' })
  @ApiParam({ name: 'problem_id', type: 'number' })
  async findUserProblemSubmission(
    @Param('problem_id') problemId: number,
    @Res() res: Response,
    @Req() req: IRequest,
  ) {
    const user_id = req.user.id;
    const data = await this.submissionsService.findProblemUserSubmission(
      user_id,
      problemId,
    );
    return fetchResponseFunc(res, data, data.message);
